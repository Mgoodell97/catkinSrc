...
t_m = 1
joy_target = [0,0]
min_dis = 1.2   # minimum distance to stay away from object for open sector
...
def distSensor_callback(msg):
    global ext_height, ext_lpf, pos_z, z_cmd, past_z_err
    if mode == 1: # sim
        ground_range = msg
    else: # mavros and DJI
    ...
    if mode == 3: # mavros
        ...
        past_z_err = z_err
        # cap z linear cmd
        if z_cmd > 6:  # max accent
            z_cmd = 6
        elif z_cmd < -4:  # max decent
            z_cmd = -4
...
def set_waypoint(msg):
    global waypoint, hold_flag, heading_nxt_wp, z_desired, pos_z
...
def callback_avoid_agents(msg):
    global yaw, agent_location, zero_vel_flag, close_timeout_flag, time
    if mode == 1:
        # get agent number
