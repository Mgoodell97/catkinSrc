...
void cmdPositionSetpoint(
  ...
  void run()
  {
    ...
    m_pubPos = n.advertise<geometry_msgs::PoseStamped>(m_tf_prefix + "/pose", 10);
    m_pubAng = n.advertise<geometry_msgs::Vector3>(m_tf_prefix + "/angle", 10);
    ...
    std::unique_ptr<LogBlock<logPos> > logBlockPos;
    std::unique_ptr<LogBlock<logAng> > logBlockAng;
    ...
    if (m_enableLogging) {
      ...
      {
        std::function<void(uint32_t, logPos*)> cbPos = std::bind(&CrazyflieROS::onLogPosData, this, std::placeholders::_1, std::placeholders::_2);

        logBlockPos.reset(new LogBlock<logPos>(
          &m_cf,{
            {"stateEstimate", "x"},
            {"stateEstimate", "y"},
            {"stateEstimate", "z"},
          }, cbPos));
        logBlockPos->start(1); // 10ms
      }

      {
        std::function<void(uint32_t, logAng*)> cbAng = std::bind(&CrazyflieROS::onLogAngData, this, std::placeholders::_1, std::placeholders::_2);

        logBlockAng.reset(new LogBlock<logAng>(
          &m_cf,{
            {"stabilizer", "roll"},
            {"stabilizer", "pitch"},
            {"stabilizer", "yaw"},
          }, cbAng));
        logBlockAng->start(1); // 10ms
      }
  ...
  void onLogPosData(uint32_t time_in_ms, logPos* data) {
  ...
  void onLogAngData(uint32_t time_in_ms, logAng* data) {
    geometry_msgs::Vector3 msg;
    msg.x = data->roll;
    msg.y = data->pitch;
    msg.z = data->yaw;
    m_pubAng.publish(msg);
