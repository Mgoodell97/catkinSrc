...
double absYawOrientation(double currentYaw){
  ...
  absYaw = absYaw + yawAbsCount*2*M_PI;
...
void pose_cb(const geometry_msgs::PoseStamped::ConstPtr& poseMsg){
  ...
  fixedYaw = absYawOrientation(yaw);
...
int main(int argc, char **argv){
  ...
  while((ros::ok() && !current_state.connected) || !controller_cb_flag || !state_cb_flag || !pose_cb_flag){
    ros::spinOnce();
    rate.sleep();
    if (controller_cb_flag && state_cb_flag && pose_cb_flag){
      break;
    }
  }
  ...
  DesiredVel.twist.linear.x = joyStickValues.axes[4];
  DesiredVel.twist.linear.y = joyStickValues.axes[3];
  ...
  while(ros::ok()){
    DesiredVel.twist.linear.x = scaleFactor*joyStickValues.axes[4];
    DesiredVel.twist.linear.y = scaleFactor*joyStickValues.axes[3];
    DesiredVel.twist.linear.z = scaleFactor*joyStickValues.axes[1];
    DesiredVel.twist.angular.z = scaleFactor*joyStickValues.axes[0];
    ROS_INFO("Yaw: %f   yawFrame: %i  yawAbsCount: %i",fixedYaw, yawFrame,yawAbsCount);
