...
int main(int argc, char **argv){
  ros::init(argc, argv, "xboxAltControl");
  ros::NodeHandle nh;
  ...
  ros::Publisher local_att_pub = nh.advertise<geometry_msgs::TwistStamped>("mavros/setpoint_attitude/cmd_vel",1);
  ros::Publisher local_att_thrust = nh.advertise<mavros_msgs::Thrust>("mavros/setpoint_attitude/thrust",1);
  ...
  geometry_msgs::TwistStamped DesiredAtt;
  mavros_msgs::Thrust DesiredAttThrust;

  double scaleFactor = 3.14;
  DesiredAtt.twist.angular.x = -scaleFactor*joyStickValues.axes[4];
  DesiredAtt.twist.angular.y = -scaleFactor*joyStickValues.axes[3];
  DesiredAtt.twist.angular.z = -scaleFactor*joyStickValues.axes[0];
  //map(value, fromLow, fromHigh, toLow, toHigh)
  DesiredAttThrust.thrust = map(joyStickValues.axes[1],-1,1,0.001,1);
  ...
  for(int i = 100; ros::ok() && i > 0; --i){
    local_att_pub.publish(DesiredAtt);
    local_att_thrust.publish(DesiredAttThrust);
    ros::spinOnce();
    rate.sleep();
  }
  ...
  while(ros::ok()){
    DesiredAtt.twist.angular.x = -scaleFactor*joyStickValues.axes[4];
    DesiredAtt.twist.angular.y = -scaleFactor*joyStickValues.axes[3];
    DesiredAtt.twist.angular.z = joyStickValues.axes[0];
    //map(value, fromLow, fromHigh, toLow, toHigh)
    DesiredAttThrust.thrust = 0.75*map(joyStickValues.axes[1],-1,1,0.001,1);
