...
def initFunc():
    ...
    DesiredVel.twist.linear.x = controllerValues.axes[4]
    DesiredVel.twist.linear.y = controllerValues.axes[3]
    DesiredVel.twist.linear.z = controllerValues.axes[1]
...
def main():
    ...
    while not rospy.is_shutdown():
        ...
        DesiredVel.twist.linear.y = controllerValues.axes[3]
        DesiredVel.twist.linear.z = controllerValues.axes[1]
        DesiredVel.twist.angular.z = controllerValues.axes[0]

        if ( not current_state.mode == "OFFBOARD" and (rospy.get_rostime() - last_request) > rospy.Duration.from_sec(2.0)):
            modeResponse = set_mode_client(0,"OFFBOARD")
            if (modeResponse.mode_sent):
                rospy.loginfo("Offboard enabled")
            last_request = rospy.get_rostime()
        else:
            if (not current_state.armed and ((rospy.get_rostime() - last_request) > rospy.Duration.from_sec(2.0))):
