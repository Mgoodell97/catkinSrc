...
def main():
    ...
    while not rospy.is_shutdown():
        ...
        if i > 200 and i < 500:
            Xfound = np.array([0.2], dtype=np.float32)
            Yfound = np.array([0.2], dtype=np.float32)
            ...
            Xfound = np.array([0.2, 2.7], dtype=np.float32)
            Yfound = np.array([0.2, 0.3], dtype=np.float32)
