...
def particles_cb(particles_msg):
    global particlesCent
    particlesCent = particles_msg

def plume_pose_cb(pose_cb_msg):
    global plume_pose
    plume_pose = pose_cb_msg
...
def main():
    ...
    rospy.Subscriber("particles", particles, particles_cb)
    rospy.Subscriber("estimatedGaussian", estimatedGaussian, gaussian_cb)

    rospy.Subscriber("/mocap_node/PlumeSource/pose", PoseStamped, plume_pose_cb)
    ...
    xPlumePlot = np.arange(minLimX, maxLimX+1, 0.05)
    yPlumePlot = np.arange(minLimY, maxLimY+1, 0.05)
    ...
    while not rospy.is_shutdown():
        ...
        plt.plot(particlesCent.X,particlesCent.Y,'b.')
        plt.plot(plume_pose.pose.position.x,plume_pose.pose.position.y,'rx',  markersize=9)
        ...
        try:
            ...
            for xCurrentIndex in range(len(xPlumePlot)):
                for yCurrentIndex in range(len(yPlumePlot)):
                    conArray[yCurrentIndex,xCurrentIndex] = getReading(xPlumePlot[xCurrentIndex], yPlumePlot[yCurrentIndex], estimatedGaussianCent.Theta, estimatedGaussianCent.X, estimatedGaussianCent.Y, estimatedGaussianCent.Z, estimatedGaussianCent.Q, estimatedGaussianCent.V, estimatedGaussianCent.Dy, estimatedGaussianCent.Dz)
        ...
        if SpawnUAV1 == 1:
            plt.plot(xPltRobot1, yPltRobot1,"k")
            plt.plot(Robot1_poseXft, Robot1_poseYft,'kD',  markersize=6)

        if SpawnUAV2 == 1:
            plt.plot(xPltRobot2, yPltRobot2,"k")
            plt.plot(Robot2_poseXft, Robot2_poseYft,'kD',  markersize=6)

        if SpawnUAV3 == 1:
            plt.plot(xPltRobot3, yPltRobot3,"k")
            plt.plot(Robot3_poseXft, Robot3_poseYft,'kD',  markersize=6)

        if SpawnUAV4 == 1:
            plt.plot(xPltRobot4, yPltRobot4,"k")
            plt.plot(Robot4_poseXft, Robot4_poseYft,'kD',  markersize=6)

        if SpawnUAV5 == 1:
            plt.plot(xPltRobot5, yPltRobot5,"k")
            plt.plot(Robot5_poseXft, Robot5_poseYft,'kD',  markersize=6)
