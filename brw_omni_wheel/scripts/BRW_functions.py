...
def moveRobot(xOld, yOld, step=1, minBoundX=-25, maxBoundX=25, minBoundY=-25, maxBoundY=25):
 
    direction = randomNumAtoB(-2*pi,2*pi)
    # step = 1
    xStep = step * np.cos(direction)
    yStep = step * np.sin(direction)

    xOld += xStep
    yOld += yStep
    
    xNew = wapointBoundsLimiter(xOld,minBoundX,maxBoundX)
    yNew = wapointBoundsLimiter(yOld,minBoundY,maxBoundY)    
    
    return xNew, yNew


def biasedRandomWalk(xRobotNewFunc, yRobotNewFunc, previousReading, currentReading, biasRange, previousBias, step=1, minBoundX=-25, maxBoundX=25, minBoundY=-25, maxBoundY=25, debug = False, LimitBounds = True):
    slope = currentReading - previousReading
    
    if slope == 0: # no slope found go in random direction
        xNew, yNew = moveRobot(xRobotNewFunc, yRobotNewFunc, step, minBoundX, maxBoundX, minBoundY, maxBoundY)
        ...
        if slope > 0:
            ...
            xStep = step * np.cos(biasDirection)
            yStep = step * np.sin(biasDirection)
        
            xRobotNewFunc += xStep
            yRobotNewFunc += yStep            
            
            if LimitBounds:
                xNew = wapointBoundsLimiter(xRobotNewFunc, minBoundX, maxBoundX)
                yNew = wapointBoundsLimiter(yRobotNewFunc, minBoundY, maxBoundY) 
            else:
                xNew = xRobotNewFunc
                yNew = yRobotNewFunc
            if debug == True:
...
class biasRandomWalkSim:
    def __init__(self, simStepsClass, biasRangeClass, thetaPlumeClass, xPlumeClass, yPlumeClass, QPlume, vPlume, DyPlume, DzPlume, xRobotSpawnClass, yRobotSpawnClass, minLimClass = -25, maxLimClass = 25, debug = False, LimitBounds = True):
        ...
        # Get initial reading
        previousReading = getReading(xRobotOld, yRobotOld, self.thetaPlume, self.xPlume, self.yPlume, 0.0, self.QPlume, self.vPlume, self.DyPlume, self.DzPlume)  
        previousReading = (1/self.vPlume) * (1/area) * 1000 * previousReading
    ...
    def plotSim(self, frameTime, debug = False, FixedSize = False):
        ...
        val = input("Press enter to start ") 
