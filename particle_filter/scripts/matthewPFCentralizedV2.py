...
def observerEquation(robotPose, xp):
    ...
    for i in range(N):
        ...
        # yp[i] = getReading(robotPose[0], robotPose[1], xp[i,3], xp[i,0], xp[i,1], xp[i,2] - robotPose[2], xp[i,4], xp[i,5], xp[i,6], xp[i,7])
    return yp
...
def main():
    ...
    while not rospy.is_shutdown(): # while roscore is running do this. if not, stop... Executes Function
        ...
        if plumeType == 1:
            chem_reading1 = robotSensorReadingPoseGauss_R1.ppm
            robotPoseNew1 = np.array([robotSensorReadingPoseGauss_R1.x, robotSensorReadingPoseGauss_R1.y, robotSensorReadingPoseGauss_R1.z])
            yi_1 = observerEquation(robotPoseNew1, xp)

            chem_reading2 = robotSensorReadingPoseGauss_R2.ppm
            robotPoseNew2 = np.array([robotSensorReadingPoseGauss_R2.x, robotSensorReadingPoseGauss_R2.y, robotSensorReadingPoseGauss_R2.z])
            yi_2 = observerEquation(robotPoseNew2, xp)

            chem_reading3 = robotSensorReadingPoseGauss_R3.ppm
            robotPoseNew3 = np.array([robotSensorReadingPoseGauss_R3.x, robotSensorReadingPoseGauss_R3.y, robotSensorReadingPoseGauss_R3.z])
            yi_3 = observerEquation(robotPoseNew3, xp)

            chem_reading4 = robotSensorReadingPoseGauss_R4.ppm
            robotPoseNew4 = np.array([robotSensorReadingPoseGauss_R4.x, robotSensorReadingPoseGauss_R4.y, robotSensorReadingPoseGauss_R4.z])
            yi_4 = observerEquation(robotPoseNew4, xp)

            chem_reading5 = robotSensorReadingPoseGauss_R5.ppm
            robotPoseNew5 = np.array([robotSensorReadingPoseGauss_R5.x, robotSensorReadingPoseGauss_R5.y, robotSensorReadingPoseGauss_R5.z])
            yi_5 = observerEquation(robotPoseNew5, xp)

        elif plumeType == 2:
            chem_reading1 = robotSensorReadingPoseGADEN_R1.raw
            robotPoseNew1 = np.array([robotSensorReadingPoseGADEN_R1.local_x, robotSensorReadingPoseGADEN_R1.local_y, robotSensorReadingPoseGADEN_R1.local_z])
            yi_1 = observerEquation(robotPoseNew1, xp)

            chem_reading2 = robotSensorReadingPoseGADEN_R2.raw
            robotPoseNew2 = np.array([robotSensorReadingPoseGADEN_R2.local_x, robotSensorReadingPoseGADEN_R2.local_y, robotSensorReadingPoseGADEN_R2.local_z])
            yi_2 = observerEquation(robotPoseNew2, xp)

            chem_reading3 = robotSensorReadingPoseGADEN_R3.raw
            robotPoseNew3 = np.array([robotSensorReadingPoseGADEN_R3.local_x, robotSensorReadingPoseGADEN_R3.local_y, robotSensorReadingPoseGADEN_R3.local_z])
            yi_3 = observerEquation(robotPoseNew3, xp)

            chem_reading4 = robotSensorReadingPoseGADEN_R4.raw
            robotPoseNew4 = np.array([robotSensorReadingPoseGADEN_R4.local_x, robotSensorReadingPoseGADEN_R4.local_y, robotSensorReadingPoseGADEN_R4.local_z])
            yi_4 = observerEquation(robotPoseNew4, xp)

            chem_reading5 = robotSensorReadingPoseGADEN_R5.raw
            robotPoseNew5 = np.array([robotSensorReadingPoseGADEN_R5.local_x, robotSensorReadingPoseGADEN_R5.local_y, robotSensorReadingPoseGADEN_R5.local_z])
            yi_5 = observerEquation(robotPoseNew5, xp)

        elif plumeType == 3:
            chem_reading1 = robotSensorReadingPoseMPS_R1.pressure
            robotPoseNew1 = np.array([robotSensorReadingPoseMPS_R1.local_x, robotSensorReadingPoseMPS_R1.local_y, robotSensorReadingPoseMPS_R1.local_z])
            yi_1 = observerEquation(robotPoseNew1, xp)

            chem_reading2 = robotSensorReadingPoseMPS_R2.pressure
            robotPoseNew2 = np.array([robotSensorReadingPoseMPS_R2.local_x, robotSensorReadingPoseMPS_R2.local_y, robotSensorReadingPoseMPS_R2.local_z])
            yi_2 = observerEquation(robotPoseNew2, xp)

            chem_reading3 = robotSensorReadingPoseMPS_R3.pressure
            robotPoseNew3 = np.array([robotSensorReadingPoseMPS_R3.local_x, robotSensorReadingPoseMPS_R3.local_y, robotSensorReadingPoseMPS_R3.local_z])
            yi_3 = observerEquation(robotPoseNew3, xp)

            chem_reading4 = robotSensorReadingPoseMPS_R4.pressure
            robotPoseNew4 = np.array([robotSensorReadingPoseMPS_R4.local_x, robotSensorReadingPoseMPS_R3.local_y, robotSensorReadingPoseMPS_R4.local_z])
            yi_4 = observerEquation(robotPoseNew4, xp)

            chem_reading5 = robotSensorReadingPoseMPS_R5.pressure
            robotPoseNew5 = np.array([robotSensorReadingPoseMPS_R5.local_x, robotSensorReadingPoseMPS_R5.local_y, robotSensorReadingPoseMPS_R5.local_z])
            yi_5 = observerEquation(robotPoseNew5, xp)

        print("Robot:", robotPoseNew1," data: ", chem_reading1)
        ...
        wp_1 = wp * norm.pdf(chem_reading1, yi_1, r)
        wp_2 = wp * norm.pdf(chem_reading2, yi_2, r)
        wp_3 = wp * norm.pdf(chem_reading3, yi_3, r)
        wp_4 = wp * norm.pdf(chem_reading4, yi_4, r)
        wp_5 = wp * norm.pdf(chem_reading5, yi_5, r)

        wp = wp_1*wp_2*wp_3*wp_4*wp_5
