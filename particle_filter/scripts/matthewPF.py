...
robotSensorReadingPose = gaussian() # chem reading in gauss env
...
def gaussSensorAndPose_cb(gaussMsg):
    global robotSensorReadingPose
    global sensorMsg_cb_flag
    robotSensorReadingPose = gaussMsg
...
def main():
    ...
    #Generate particles
    xp     = np.random.uniform(X_l     , X_u     , NumOfParticles)
    yp     = np.random.uniform(Y_l     , Y_u     , NumOfParticles)
    zp     = np.random.uniform(Z_l     , Z_u     , NumOfParticles)
    thetap = np.random.uniform(Theta_l , Theta_u , NumOfParticles)
    Qp     = np.random.uniform(Q_l     , Q_u     , NumOfParticles)
    vp     = np.random.uniform(V_l     , V_u     , NumOfParticles)
    Dyp    = np.random.uniform(Dy_l    , Dy_u    , NumOfParticles)
    Dzp    = np.random.uniform(Dz_l    , Dz_u    , NumOfParticles)

    # Turn them into matrix's
    xp     = xp[:,np.newaxis]
    yp     = yp[:,np.newaxis]
    zp     = zp[:,np.newaxis]
    thetap = thetap[:,np.newaxis]
    Qp     = Qp[:,np.newaxis]
    vp     = vp[:,np.newaxis]
    Dyp    = Dyp[:,np.newaxis]
    Dzp    = Dzp[:,np.newaxis]
    ...
    while not rospy.is_shutdown(): # while roscore is running do this. if not, stop... Executes Function
        ...
        # Get chemicla reading
        chem_reading = robotSensorReadingPose.ppm
        robotPose = [robotSensorReadingPose.x, robotSensorReadingPose.y, robotSensorReadingPose.z]

        # 1. Measurement prediction
        yi = observerEquation(robotPose, xp)
